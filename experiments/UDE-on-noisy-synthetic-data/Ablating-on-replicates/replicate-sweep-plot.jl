...
allocs = sort(unique(df.alloc); by = a -> (t=parse.(Int, split(a,"×")); (-t[1], t[2])))
# T descending, m ascending → 20×3,20×4,20×5,20×6, 10×3,…, 5×6
...
function plot_error_vs_noise(err_cols, glabels, fname, suptitle)
    ...
    colors = Makie.categorical_colors(:tab20, length(allocs))
